                ...
                {groupItems.map((item) => (
                  ...
                  >
                    {({ isActive }) => (
                      <>
                        {/*
                          A barra que marca onde a pessoa está.

                          Posicionada por cima em vez de `border-l`: borda
                          ocuparia espaço e empurraria o ícone e o texto um
                          pouco para a direita só no item ativo, fazendo a lista
                          inteira tremer a cada clique.

                          Usa `primary-ink`, não `primary`: sobre a pastilha de
                          10% a cor crua some quando a empresa escolhe um tom
                          claro — a mesma razão do texto ao lado.
                        */}
                        {isActive ? (
                          <span
                            aria-hidden
                            className="absolute left-0 top-1/2 h-5 w-[3px] -translate-y-1/2 rounded-r-full bg-primary-ink"
                          />
                        ) : null}
